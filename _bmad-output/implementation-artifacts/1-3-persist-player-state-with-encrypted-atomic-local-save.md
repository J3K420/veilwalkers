...
- [ ] **Task 8 — Verify, update story/sprint records, and commit**
  ...
  - [ ] **Manual Bootstrap play check (the one path with no EditMode coverage):** enter Play Mode in the Bootstrap scene once and confirm via the Console that GameServices wires + seals and a save file appears under `Application.persistentDataPath`; ask James to run this if the Editor is needed interactively.
