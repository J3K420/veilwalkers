...
namespace Veilwalkers.UI
{
    ...
    public sealed class MaterializationView : MonoBehaviour
    {
        ...
        // TODO(Epic 6): read the real reduced-motion / photosensitivity setting (a SaveModel flag
        // or a player-pref) instead of this stub. The presenter already tames the plan when true.
        ...
        {
            MaterializationPlan plan = _presenter.PlanFor(monsterId, tier, _reducedMotion);
            Render(plan);
        }
        ...
        // branded camera glitch when plan.BrandedGlitch (tamed to static when plan.ReducedMotion),
        // the ambiance tokens from plan.Ambiance, keep the action bar hidden until settle
        // (presenter.ShouldShowActionBar), and NEVER block input recovery (plan.InputRecoveryBlocked
        // is always false).
        private void Render(MaterializationPlan plan)
        {
            GameLog.Info(
                $"MaterializationView: '{plan.MonsterId}' ({plan.Tier}) → {plan.Variant} over " +
                $"{plan.DurationSeconds:0.##}s, ambiance {plan.Ambiance.Intensity}" +
